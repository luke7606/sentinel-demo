...
const PROJECT_TEMPLATES = {
  client: {
    label: "Cliente", labelEn: "Client", icon: "🏢", color: "#6366f1",
    defaultStatus: "on-track", defaultHealth: 100,
    milestones: ["Discovery & Kickoff","Development Phase 1","Development Phase 2","Testing & QA","Launch & Handoff"],
    fields: ["client","budget","dueDate"],
  },
  pm: {
    label: "Project Management", labelEn: "Project Management", icon: "📋", color: "#8b5cf6",
    defaultStatus: "on-track", defaultHealth: 100,
    milestones: ["Planning & Scope","Sprint 1","Sprint 2","Sprint 3","Delivery"],
    fields: ["client","budget","dueDate"],
  },
  hr: {
    label: "Recursos Humanos", labelEn: "Human Resources", icon: "👥", color: "#f59e0b",
    defaultStatus: "on-track", defaultHealth: 100,
    milestones: ["Needs Assessment","Process Design","Implementation","Training","Go-live"],
    fields: ["client","dueDate"],
  },
  finance: {
    label: "Finanzas", labelEn: "Finance", icon: "💰", color: "#10b981",
    defaultStatus: "on-track", defaultHealth: 100,
    milestones: ["Audit & Analysis","Budget Planning","Reporting Setup","Review","Close"],
    fields: ["client","budget","dueDate"],
  },
  support: {
    label: "Soporte / IT", labelEn: "Support / IT", icon: "🛠", color: "#ef4444",
    defaultStatus: "on-track", defaultHealth: 100,
    milestones: ["Triage","Root Cause Analysis","Fix Implementation","Testing","Sign-off"],
    fields: ["client","dueDate"],
  },
};
...
const LANG = {
  en: {
    ...
    addToKb:"Add to knowledge base", indexedDocs:"Indexed documents", noDocsYet:"No documents yet — upload something to get started",
    ...
    // Integrations
    integrationsTitle:"Integrations", integrationsSub:"Connect external channels to Sentinel",
    whatsappInt:"WhatsApp", whatsappDesc:"Send ticket alerts via WhatsApp",
    emailInt:"Email (EmailJS)", emailDesc:"Send email notifications on escalation",
    whatsappConfig:"WhatsApp number (intl format)", emailConfig:"EmailJS Service ID",
    saveConfig:"Save", configSaved:"✓ Saved",
    notifyWhatsapp:"📱 Notify via WhatsApp", notifyEmail:"📧 Notify via Email",
    // Status
    ...
    sysInternal:{pm:"You are Sentinel PM Assistant. Help with project tracking, sprints, milestones and team velocity. Be concise.",hr:"You are Sentinel HR Assistant. Help with team hours, headcount and people metrics.",finance:"You are Sentinel Finance Assistant. Help with budget tracking, costs and forecasting. Be precise.",support:"You are Sentinel Support Assistant. Help diagnose issues, analyze tickets and suggest fixes."},
  },
  es: {
    ...
    addToKb:"Agregar a la base de conocimiento", indexedDocs:"Documentos indexados", noDocsYet:"Sin documentos aún — subí algo para empezar",
    ...
    whatsappInt:"WhatsApp", whatsappDesc:"Enviá alertas de tickets por WhatsApp",
    emailInt:"Email (EmailJS)", emailDesc:"Notificaciones por email al escalar",
    whatsappConfig:"Número WhatsApp (formato intl.)", emailConfig:"EmailJS Service ID",
    saveConfig:"Guardar", configSaved:"✓ Guardado",
    notifyWhatsapp:"📱 Notificar por WhatsApp", notifyEmail:"📧 Notificar por Email",
    ...
    sysClient:(proj,name,docs)=>`Sos Sentinel, IA de soporte para el proyecto "${proj}" (cliente: ${name}). Español, técnico, conciso. Máximo 3 pasos. Si no se resuelve en 3 intentos, empezá con ESCALAR_N2.\n${docs?`DOCS:\n${docs}`:"Sin docs."}`,
    sysAdmin:(projects,docs)=>`Sos Sentinel, IA ejecutiva para la agencia Dramhost.\nProyectos:\n${projects}\n${docs?`Docs:\n${docs}`:""}`,
    sysInternal:{pm:"Sos el Asistente PM de Sentinel. Ayudá con seguimiento, sprints y milestones.",hr:"Sos el Asistente RRHH de Sentinel. Ayudá con horas, headcount y métricas de personas.",finance:"Sos el Asistente de Finanzas de Sentinel. Presupuesto, costos y proyecciones.",support:"Sos el Asistente de Soporte de Sentinel. Diagnosticá problemas y sugerí soluciones."},
  }
};
...
const DEMO_PROJECTS = {
  "nova-commerce": {
    id:"nova-commerce", name:"Nova Commerce Platform", client:"RetailCo Inc.", type:"client",
    color:"#6366f1", status:"on-track", health:87, budget:120000, spent:94000,
    startDate:"2025-01-15", dueDate:"2025-06-30",
    milestones:[{id:"m1",name:"Discovery & Architecture",due:"2025-02-15",status:"done"},{id:"m2",name:"Core Backend APIs",due:"2025-03-30",status:"done"},{id:"m3",name:"Frontend MVP",due:"2025-04-30",status:"in-progress"},{id:"m4",name:"Integrations & Testing",due:"2025-05-31",status:"pending"},{id:"m5",name:"Launch & Handoff",due:"2025-06-30",status:"pending"}],
    team:["Alex Rivera","Sam Chen","Jordan Lee","Taylor Kim"],
    docs:[{id:"d1",name:"Tech Stack & Runbook",type:"text",source:"manual",uploadedAt:"2025-02-01",content:`Project: Nova Commerce Platform | Client: RetailCo Inc.\nStack: Node.js 20, React 18, PostgreSQL 15, Redis 7, AWS EC2\nPAYMENTS: Stripe v3 · webhook /api/payments/webhook · STRIPE_SECRET env · 30s timeout\nINVENTORY: cron every 5min · Redis TTL 300s\nINFRA: AWS EC2 + PM2 · deploy: ./scripts/deploy.sh`}],
    tickets:[],
    activity:[{type:"commit",text:"feat: add Stripe webhook handler",user:"Alex Rivera",time:"2h ago"},{type:"task",text:"Frontend MVP — Sprint 4 started",user:"Sam Chen",time:"5h ago"}],
  },
  "fleet-tracker": {
    id:"fleet-tracker", name:"Fleet Tracker 360", client:"LogiCorp SA", type:"client",
    color:"#10b981", status:"at-risk", health:61, budget:85000, spent:71000,
    startDate:"2025-02-01", dueDate:"2025-07-15",
    milestones:[{id:"m1",name:"GPS Integration Layer",due:"2025-03-01",status:"done"},{id:"m2",name:"Real-time Dashboard",due:"2025-04-15",status:"at-risk"},{id:"m3",name:"Mobile App Beta",due:"2025-05-30",status:"pending"},{id:"m4",name:"Reporting Module",due:"2025-06-30",status:"pending"}],
    team:["Morgan Walsh","Casey Park"],
    docs:[{id:"d2",name:"Architecture Overview",type:"text",source:"manual",uploadedAt:"2025-02-10",content:`Project: Fleet Tracker 360 | Client: LogiCorp SA\nStack: Python FastAPI, React Native, PostgreSQL, Redis, Google Maps API\nGPS: WebSocket stream · update every 30s\nMOBILE: React Native 0.73 · Expo`}],
    ...
    activity:[{type:"alert",text:"Milestone 2 at risk — 3 tasks overdue",user:"System",time:"30m ago"},{type:"commit",text:"fix: websocket reconnection logic",user:"Morgan Walsh",time:"3h ago"}],
  },
  "hr-portal": {
    id:"hr-portal", name:"HR Self-Service Portal", client:"Internal — Dramhost", type:"hr",
    color:"#f59e0b", status:"on-track", health:92, budget:45000, spent:28000,
    startDate:"2025-03-01", dueDate:"2025-07-01",
    milestones:[{id:"m1",name:"Auth & Roles",due:"2025-03-31",status:"done"},{id:"m2",name:"Leave Management",due:"2025-04-30",status:"done"},{id:"m3",name:"Payroll Integration",due:"2025-05-31",status:"in-progress"}],
...
function NewProjectModal({t,lang,onClose,onCreate}) {
  ...
  const handleCreate = () => {
    ...
    const newProj = {
      id, name, client: client||"TBD", type: tmpl,
      color: tpl.color, status: "on-track", health: 100,
      budget: parseInt(budget)||0, spent: 0,
      startDate: new Date().toISOString().split("T")[0],
      dueDate: dueDate||"",
      milestones: tpl.milestones.map((m,i)=>({id:`m${i+1}`,name:m,due:"",status:"pending"})),
      team:[], docs:[], tickets:[],
      activity:[{type:"task",text:`Project created via Sentinel`,user:"Admin",time:"just now"}],
    };
    onCreate(newProj);
    onClose();
  };
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999}}>
      <div style={{background:"#0d1526",border:"1px solid #334155",borderRadius:16,padding:28,width:480,maxWidth:"95vw",boxShadow:"0 0 60px rgba(99,102,241,0.2)"}}>
        ...
        <div style={{marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6}}>
            {Object.entries(PROJECT_TEMPLATES).map(([key,tpl])=>(
              <button key={key} onClick={()=>setTmpl(key)} style={{...S.smBtn,flexDirection:"column",alignItems:"center",padding:"8px 4px",gap:3,...(tmpl===key?{borderColor:tpl.color,color:tpl.color,background:"#1e293b"}:{})}}>
                <span style={{fontSize:16}}>{tpl.icon}</span>
                <span style={{fontSize:9,textAlign:"center",lineHeight:1.2}}>{lang==="es"?tpl.label:tpl.labelEn}</span>
              </button>
            ))}
          </div>
        ...
        <div style={{marginTop:14,padding:"10px 12px",background:"#1e293b",borderRadius:8}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
            {tpl.milestones.map((m,i)=><span key={i} style={{fontSize:10,background:"#0d1526",color:"#94a3b8",padding:"2px 8px",borderRadius:4,border:"1px solid #334155"}}>{m}</span>)}
          </div>
        </div>
...
// ── INTEGRATIONS PANEL ─────────────────────────────────────────
function IntegrationsPanel({t,lang}) {
  const [waNum, setWaNum]       = useState("");
  const [emailSvc, setEmailSvc] = useState("");
  const [waSaved, setWaSaved]   = useState(false);
  const [emSaved, setEmSaved]   = useState(false);

  const saveWa = () => { if(waNum) { localStorage.setItem("sentinel_wa",waNum); setWaSaved(true); setTimeout(()=>setWaSaved(false),2000); }};
  const saveEm = () => { if(emailSvc) { localStorage.setItem("sentinel_em",emailSvc); setEmSaved(true); setTimeout(()=>setEmSaved(false),2000); }};
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <div style={S.card}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            ...
            <div><div style={{fontSize:13,fontWeight:700,color:"#e2e8f0"}}>{t.whatsappInt}</div><div style={{fontSize:11,color:"#64748b"}}>{t.whatsappDesc}</div></div>
          </div>
          <label style={S.label}>{t.whatsappConfig}</label>
          <div style={{display:"flex",gap:6}}>
            <input style={{...S.input,flex:1}} placeholder="+54911..." value={waNum} onChange={e=>setWaNum(e.target.value)}/>
            <button onClick={saveWa} style={S.smBtn}>{waSaved?t.configSaved:t.saveConfig}</button>
          </div>
          <div style={{fontSize:10,color:"#475569",marginTop:8}}>Tickets will open wa.me with pre-filled context</div>
        </div>
        {/* Email */}
        <div style={S.card}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            <span style={{fontSize:20}}>📧</span>
            <div><div style={{fontSize:13,fontWeight:700,color:"#e2e8f0"}}>{t.emailInt}</div><div style={{fontSize:11,color:"#64748b"}}>{t.emailDesc}</div></div>
          </div>
          <label style={S.label}>{t.emailConfig}</label>
          <div style={{display:"flex",gap:6}}>
            <input style={{...S.input,flex:1}} placeholder="service_xxxxxxx" value={emailSvc} onChange={e=>setEmailSvc(e.target.value)}/>
            <button onClick={saveEm} style={S.smBtn}>{emSaved?t.configSaved:t.saveConfig}</button>
          </div>
          <a href="https://www.emailjs.com" target="_blank" rel="noreferrer" style={{fontSize:10,color:"#818cf8",marginTop:8,display:"block"}}>Get free EmailJS account →</a>
        </div>
      </div>
    </div>
  );
...
export default function Sentinel() {
  ...
  const notifyWhatsApp = (tkt) => {
    const waNum = localStorage.getItem("sentinel_wa");
    if (!waNum) return;
    const msg = encodeURIComponent(`🚨 Sentinel Alert\nTicket: ${tkt.id}\nProject: ${projects[pid]?.name}\nIssue: ${tkt.summary}`);
  ...
  const addUrl=()=>{
    ...
    const doc={id:Date.now(),name:urlInput,type:"url",source:"url",content:`[URL: ${urlInput}]`,uploadedAt:new Date().toISOString().split("T")[0]};
    setProjects(p=>({...p,[pid]:{...p[pid],docs:[...(p[pid]?.docs||[]),doc]}})); setUrlInput("");
  };
  const addScript=()=>{
    ...
    setProjects(p=>({...p,[pid]:{...p[pid],docs:[...(p[pid]?.docs||[]),doc]}})); setScriptTxt("");
  };
      ...
      <aside style={S.sidebar}>
        ...
        {user.role!=="client" && (
          <div style={{marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",...S.label}}><span>{t.activeProject}</span>
      ...
      <main style={S.main}>
        ...
        {view==="dashboard" && (
          <div style={S.panel}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22}}>
              ...
              {user.role==="admin" && (
                <button onClick={()=>setShowNewProj(true)} style={{...S.smBtn,borderColor:"#4f46e5",color:"#818cf8"}}>
                  <I.Plus/> {lang==="es"?"Nuevo Proyecto":"New Project"}
                </button>
              )}
            </div>
                ...
                <div key={proj.id} style={{...S.card,cursor:"pointer"}} onClick={()=>{setActive(proj.id);setView("chat");}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                    ...
                    <div style={{display:"flex",gap:4,alignItems:"center"}}>
                      {proj.type && <span style={{fontSize:9,color:"#475569"}}>{PROJECT_TEMPLATES[proj.type]?.icon}</span>}
                  ...
                  <div style={{marginTop:10,display:"flex",flexDirection:"column",gap:4}}>
                    {proj.milestones?.slice(0,3).map(m=>(
                      <div key={m.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:11}}>
                        <span style={{color:"#94a3b8"}}>{m.name}</span><StatusBadge status={m.status} t={t}/>
                      </div>
                    ))}
                  </div>
                  {proj.activity?.length>0&&(
                    <div style={{marginTop:10,paddingTop:8,borderTop:"1px solid #1e293b"}}>
                      ...
                      {proj.activity.slice(0,2).map((a,i)=><div key={i} style={{fontSize:11,color:"#64748b",marginBottom:2}}><span style={{color:"#94a3b8"}}>{a.user}</span> · {a.text} · <span style={{color:"#475569"}}>{a.time}</span></div>)}
                    </div>
                  )}
        ...
        {view==="chat"&&(
          <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
            <div style={{padding:"12px 20px",borderBottom:"1px solid #1e293b",background:"#0d1526",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontWeight:700,fontSize:14,color:"#f1f5f9"}}>{user.role==="internal"?getAreaLabel(user.area):project?.name}</div><div style={{fontSize:11,color:"#64748b",marginTop:1}}>{project?.docs?.length||0} {t.docsIndexed} · {messages.length} {t.turns}</div></div>
              ...
              <div>
                ...
                <div style={{...S.card,marginTop:12}}>
                  ...
                  <div style={{display:"flex",gap:8,marginTop:12}}>
                    <button onClick={()=>{const waNum=localStorage.getItem("sentinel_wa");if(waNum){const msg=encodeURIComponent(`🚨 Ticket ${selectedTkt.id}\nProject: ${selectedTkt.projectName}\nIssue: ${selectedTkt.summary}`);window.open(`https://wa.me/${waNum.replace(/\D/g,"")}?text=${msg}`,"_blank");}else{alert("Configure WhatsApp number in Admin → Integrations");}}} style={{...S.smBtn,borderColor:"#25d366",color:"#25d366"}}>{t.notifyWhatsapp}</button>
                    <button onClick={()=>alert("Configure EmailJS in Admin → Integrations to send email notifications.")} style={{...S.smBtn,borderColor:"#818cf8",color:"#818cf8"}}>{t.notifyEmail}</button>
                  </div>
                </div>
        ...
        {view==="admin"&&user.role==="admin"&&(
          <div style={S.panel}>
            ...
            {adminTab==="integrations"&&<IntegrationsPanel t={t} lang={lang}/>}
