...
const LANG = {
  en: {
    ...
    groqFree: "Groq API — 100% Free", groqDesc: "No credit card · 500k tokens/day",
    ...
    keyNote: "Key never stored on any server. Session only.",
    ...
    signIn: "Sign in →", wrongCredentials: "Wrong username or password",
    demoAccounts: "Demo accounts",
    demoList: [["#6366f1","admin","admin123","Admin — all projects"],["#8b5cf6","pm","pm123","Internal PM — project tracking"],["#f59e0b","hr","hr123","Internal HR — people & hours"],["#10b981","finance","finance123","Internal Finance — budgets"],["#ef4444","support","support123","Internal Support — tickets"],["#6366f1","retailco","client123","Client — Nova Commerce"],["#10b981","logicorp","logi123","Client — Fleet Tracker"]],
    // Nav
    dashboard: "Dashboard", aiChat: "AI Chat", dataSources: "Data Sources", tickets: "Tickets", adminNav: "Admin",
    // Dashboard
    ...
    milestonesDone: "Milestones Done", completed: "completed",
    budgetUsed: "Budget Used",
    projectsTracked: "Projects Tracked", allClients: "all clients",
    atRisk: "At Risk", needAttention: "need attention",
    milestonesDue: "Milestones Due", thisMonth: "this month",
    teamMembers: "Team Members", activeDevs: "active devs",
    totalBudgetLbl: "Total Budget", contracted: "contracted",
    totalSpent: "Total Spent", toDate: "to date",
    remaining: "Remaining", available: "available",
    avgBurnRate: "Avg Burn Rate", budgetConsumed: "budget consumed",
    teamSize: "Team Size", activeMembers: "active members",
    projectsMonitored: "Projects", monitored: "monitored",
    ...
    connectBtn2: "Connect →", comingSoon: "Coming soon",
    ...
    indexedDocs: "Indexed documents", noDocsYet: "No documents yet — upload something to get started",
    // Tickets
    ticketsTitle: "N2 Tickets", ticketsSub: "Escalated cases for manual resolution",
    backBtn: "← Back", conversation: "Conversation",
    clientLabel: "Client", aiLabel: "Sentinel AI",
    suggestReply: "✨ Suggest AI reply", generating: "Generating...",
    closeResolved: "Close as resolved", noTickets: "No escalated tickets yet — that's a good sign 🎉",
    // Admin
    adminTitle: "Administration", adminSub: "Manage projects, clients and team access",
    createProject: "Create project", projectNamePh: "Project name", createBtn: "Create",
    projectsTitle: "Projects", teamAccounts: "Team accounts",
    manageBtn: "Manage →",
    // Roles
    adminRole: "Admin — Full Access", clientPortal: "Client Portal",
    ...
    // System prompts
    sysClient: (proj, name, docs) => `You are Sentinel, post-delivery support AI for project "${proj}" (client: ${name}). Rules: English, technical, concise. Max 3 steps. If unresolved in 3 attempts, start with ESCALAR_N2.\n${docs ? `PROJECT DOCS:\n${docs}` : "No docs loaded."}`,
    sysAdmin: (projects, docs) => `You are Sentinel, executive intelligence AI for Dramhost agency.\nProjects:\n${projects}\n${docs ? `Active docs:\n${docs}` : ""}`,
    sysInternal: { pm: "You are Sentinel PM Assistant. Help with project tracking, sprints, milestones and team velocity. Be concise.", hr: "You are Sentinel HR Assistant. Help with team hours, headcount and people metrics. Be empathetic.", finance: "You are Sentinel Finance Assistant. Help with budget tracking, costs and forecasting. Be precise.", support: "You are Sentinel Support Assistant. Help diagnose issues, analyze tickets and suggest fixes." },
  },
  es: {
    platform: "Plataforma de Inteligencia de Proyectos",
    groqFree: "Groq API — 100% GRATUITO", groqDesc: "Sin tarjeta · 500k tokens/día",
    ...
    groqConnected: "Groq conectado",
    usernameLabel: "Usuario", passwordLabel: "Contraseña",
    userPlaceholder: "admin / pm / hr / finance / support / retailco / logicorp",
    signIn: "Ingresar →", wrongCredentials: "Usuario o contraseña incorrectos",
    demoAccounts: "Cuentas demo",
    demoList: [["#6366f1","admin","admin123","Admin — todos los proyectos"],["#8b5cf6","pm","pm123","Interno PM — seguimiento"],["#f59e0b","hr","hr123","Interno RRHH — personas y horas"],["#10b981","finance","finance123","Interno Finanzas — presupuestos"],["#ef4444","support","support123","Interno Soporte — tickets"],["#6366f1","retailco","client123","Cliente — Nova Commerce"],["#10b981","logicorp","logi123","Cliente — Fleet Tracker"]],
    dashboard: "Dashboard", aiChat: "Chat IA", dataSources: "Fuentes de Datos", tickets: "Tickets", adminNav: "Admin",
    ...
    milestonesDone: "Milestones Listos", completed: "completados",
    budgetUsed: "Presupuesto Usado",
    projectsTracked: "Proyectos", allClients: "todos los clientes",
    atRisk: "En Riesgo", needAttention: "necesitan atención",
    milestonesDue: "Milestones", thisMonth: "este mes",
    teamMembers: "Miembros del Equipo", activeDevs: "devs activos",
    totalBudgetLbl: "Presupuesto Total", contracted: "contratado",
    totalSpent: "Total Gastado", toDate: "a la fecha",
    remaining: "Disponible", available: "restante",
    avgBurnRate: "Burn Rate Prom.", budgetConsumed: "presupuesto consumido",
    teamSize: "Tamaño del Equipo", activeMembers: "miembros activos",
    projectsMonitored: "Proyectos", monitored: "monitoreados",
    ...
    connectBtn2: "Conectar →", comingSoon: "Próximamente",
    ...
    addToKb: "Agregar a la base de conocimiento",
    indexedDocs: "Documentos indexados", noDocsYet: "Sin documentos aún — subí algo para empezar",
    ticketsTitle: "Tickets N2", ticketsSub: "Casos escalados para resolución manual",
    backBtn: "← Volver", conversation: "Conversación",
    clientLabel: "Cliente", aiLabel: "Sentinel IA",
    suggestReply: "✨ Sugerir respuesta con IA", generating: "Generando...",
    closeResolved: "Cerrar como resuelto", noTickets: "Sin tickets escalados aún — ¡eso es buena señal! 🎉",
    adminTitle: "Administración", adminSub: "Gestioná proyectos, clientes y accesos",
    createProject: "Crear proyecto", projectNamePh: "Nombre del proyecto", createBtn: "Crear",
    projectsTitle: "Proyectos", teamAccounts: "Cuentas del equipo",
    manageBtn: "Gestionar →",
    adminRole: "Admin — Acceso Total", clientPortal: "Portal del Cliente",
    ...
    sysClient: (proj, name, docs) => `Sos Sentinel, IA de soporte post-entrega para el proyecto "${proj}" (cliente: ${name}). Reglas: español, técnico, conciso. Máximo 3 pasos. Si no se resuelve en 3 intentos, empezá con ESCALAR_N2.\n${docs ? `DOCUMENTACIÓN:\n${docs}` : "Sin docs cargados."}`,
    sysAdmin: (projects, docs) => `Sos Sentinel, IA de inteligencia ejecutiva para la agencia Dramhost.\nProyectos:\n${projects}\n${docs ? `Docs activos:\n${docs}` : ""}`,
    sysInternal: { pm: "Sos el Asistente PM de Sentinel. Ayudá con seguimiento de proyectos, sprints, milestones y velocidad del equipo. Sé conciso.", hr: "Sos el Asistente RRHH de Sentinel. Ayudá con horas del equipo, headcount y métricas de personas. Sé empático.", finance: "Sos el Asistente de Finanzas de Sentinel. Ayudá con seguimiento de presupuesto, costos y proyecciones. Sé preciso.", support: "Sos el Asistente de Soporte de Sentinel. Ayudá a diagnosticar problemas, analizar tickets y sugerir soluciones." },
  }
};
...
const DEMO_PROJECTS = {
  "nova-commerce": {
    ...
    docs: [{ id:"d1", name:"Tech Stack & Runbook", type:"text", source:"manual", uploadedAt:"2025-02-01",
      content:`Project: Nova Commerce Platform | Client: RetailCo Inc.\nStack: Node.js 20, React 18, PostgreSQL 15, Redis 7, AWS EC2\nPAYMENTS: Stripe v3 · webhook /api/payments/webhook · STRIPE_SECRET env · 30s timeout\nINVENTORY: cron every 5min · Redis TTL 300s · flush: redis-cli FLUSHDB\nINFRA: AWS EC2 + PM2 · .env at root · deploy: ./scripts/deploy.sh` }],
    tickets: [],
    activity: [
  ...
  "fleet-tracker": {
    ...
    docs: [{ id:"d2", name:"Architecture Overview", type:"text", source:"manual", uploadedAt:"2025-02-10",
    ...
    tickets: [{ id:"TKT-0012", summary:"Real-time map not updating after 10min idle", status:"open", severity:"high", createdAt:"2025-04-10T10:00:00Z", conversation:[{ role:"user", content:"Map stops updating after 10 minutes of idle" }] }],
    activity: [
  ...
  "hr-portal": {
    ...
    docs: [], tickets: [],
    activity: [{ type:"task", text:"Payroll API integration — 70% complete", user:"Riley Johnson", time:"1h ago" }],
  },
};
...
function StatusBadge({ status, t }) {
  ...
  return <span style={{padding:"2px 8px",borderRadius:4,fontSize:10,fontWeight:700,background:c.bg,color:c.color,border:`1px solid ${c.border}`}}>{label}</span>;
}
...
export default function Sentinel() {
  ...
  // lang switcher — also update welcome message
  const setLang = (l) => {
    setLangState(l);
    if (user && messages.length <= 1) {
      const tNew = LANG[l];
      let welcome;
      if (user.role === "admin") welcome = tNew.welcomeAdmin(user.name, Object.keys(projects).length);
      else if (user.role === "client") welcome = tNew.welcomeClient(user.name);
      else welcome = tNew.welcomeInternal(user.name, lang==="es" ? AREA_CONFIG[user.area]?.label : AREA_CONFIG[user.area]?.labelEs);
  ...
  const login = (e) => {
    ...
    if (u && u.password === loginForm.p) {
      ...
      setView("dashboard");
      let welcome;
      if (u.role==="admin") welcome = t.welcomeAdmin(u.name, Object.keys(projects).length);
      else if (u.role==="client") welcome = t.welcomeClient(u.name);
      else welcome = t.welcomeInternal(u.name, getAreaLabel(u.area));
      setMessages([{ role:"assistant", content: welcome }]);
  ...
  const buildSystem = useCallback(() => {
    ...
    const docs = proj?.docs?.length ? proj.docs.slice(0,2).map(d=>`=== ${d.name} ===\n${d.content.slice(0,600)}`).join("\n\n") : null;
    if (!user) return "You are Sentinel AI.";
    if (user.role==="client") return t.sysClient(proj?.name, user.name, docs);
    if (user.role==="admin") {
      const allP = Object.values(projects).map(p=>`- ${p.name} (${p.client}): health ${p.health}%, status ${p.status}`).join("\n");
  ...
  const suggestReply = async (tkt) => {
    ...
    try {
      ...
      const r = await callGroq({apiKey:groqKey,system:"You are a senior technical agent. Reply concisely and actionably.",messages:[{role:"user",content:`Write a reply for this ticket.\nISSUE: ${tkt.summary}\nCONV:\n${conv}\nJust the reply text.`}],maxTokens:400});
      setTktReply(r);
  ...
  const addScript = () => {
    ...
    const doc = {id:Date.now(),name:lang==="es"?"Entrada Manual":"Manual Entry",type:"text",source:"manual",content:scriptTxt,uploadedAt:new Date().toISOString().split("T")[0]};
    setProjects(p=>({...p,[pid]:{...p[pid],docs:[...(p[pid]?.docs||[]),doc]}}));
    setScriptTxt("");
  };
      ...
      <div style={S.authCard}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div><div style={S.logoTitle}>SENTINEL</div><div style={{fontSize:11,color:"#475569"}}>{t.platform}</div></div>
          </div>
    ...
    <div style={S.bg}>
      <div style={{...S.authCard,maxWidth:400}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={S.logoBox}>S</div>
            <div><div style={S.logoTitle}>SENTINEL</div><div style={{fontSize:11,color:"#10b981"}}>● {t.groqConnected}</div></div>
          </div>
          <LangToggle lang={lang} setLang={setLangState} />
        </div>
      ...
      <aside style={S.sidebar}>
        ...
        {user.role!=="client" && (
          <div style={{marginBottom:12}}>
            <div style={S.label}>{t.activeProject}</div>
        ...
        {view==="dashboard" && (
          <div style={S.panel}>
            <div style={{marginBottom:22}}>
              <h2 style={S.panelH}>
                {user.role==="admin"    ? t.agencyOverview :
                 user.role==="client"   ? `${project?.name}` :
                 getAreaLabel(user.area)}
              </h2>
              <p style={S.panelSub}>
                {user.role==="admin"  ? `${Object.keys(projects).length} ${t.activeProjectsSub}` :
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:20}}>
              {user.role==="admin" && <>
                <MetricCard label={t.activeProjects} value={Object.keys(projects).length} sub={t.acrossClients}/>
                <MetricCard label={t.avgHealth} value={`${Math.round(Object.values(projects).reduce((s,p)=>s+p.health,0)/Object.keys(projects).length)}%`} sub={t.portfolioScore} color="#10b981"/>
                <MetricCard label={t.openTickets} value={allTickets.filter(tk=>tk.status==="open").length} sub={t.awaitingRes} color="#ef4444"/>
                <MetricCard label={t.totalBudget} value={`$${(Object.values(projects).reduce((s,p)=>s+p.budget,0)/1000).toFixed(0)}k`} sub={t.underMgmt} color="#f59e0b"/>
              </>}
              {user.role==="client" && <>
                <MetricCard label={t.projectHealth} value={`${project?.health}%`} sub={t.overallScore} color={project?.health>=80?"#10b981":"#f59e0b"}/>
                <MetricCard label={t.milestonesDone} value={`${project?.milestones?.filter(m=>m.status==="done").length}/${project?.milestones?.length}`} sub={t.completed} color="#818cf8"/>
                <MetricCard label={t.openTickets} value={project?.tickets?.filter(tk=>tk.status==="open").length||0} sub={t.awaitingRes} color="#ef4444"/>
                <MetricCard label={t.budgetUsed} value={`${Math.round((project?.spent/project?.budget)*100)}%`} sub={`$${(project?.spent/1000).toFixed(0)}k / $${(project?.budget/1000).toFixed(0)}k`} color="#f59e0b"/>
              </>}
              {user.role==="internal" && user.area==="pm" && <>
                <MetricCard label={t.projectsTracked} value={visibleProjects.length} sub={t.allClients}/>
                <MetricCard label={t.atRisk} value={visibleProjects.filter(p=>p.status==="at-risk").length} sub={t.needAttention} color="#ef4444"/>
                <MetricCard label={t.milestonesDue} value={visibleProjects.flatMap(p=>p.milestones||[]).filter(m=>m.status==="in-progress").length} sub={t.thisMonth} color="#f59e0b"/>
                <MetricCard label={t.teamMembers} value={[...new Set(visibleProjects.flatMap(p=>p.team||[]))].length} sub={t.activeDevs} color="#818cf8"/>
              </>}
              {user.role==="internal" && user.area==="finance" && <>
                <MetricCard label={t.totalBudgetLbl} value={`$${(visibleProjects.reduce((s,p)=>s+p.budget,0)/1000).toFixed(0)}k`} sub={t.contracted} color="#10b981"/>
                <MetricCard label={t.totalSpent} value={`$${(visibleProjects.reduce((s,p)=>s+p.spent,0)/1000).toFixed(0)}k`} sub={t.toDate} color="#f59e0b"/>
                <MetricCard label={t.remaining} value={`$${((visibleProjects.reduce((s,p)=>s+p.budget,0)-visibleProjects.reduce((s,p)=>s+p.spent,0))/1000).toFixed(0)}k`} sub={t.available} color="#818cf8"/>
                <MetricCard label={t.avgBurnRate} value={`${Math.round(visibleProjects.reduce((s,p)=>s+(p.spent/p.budget*100),0)/visibleProjects.length)}%`} sub={t.budgetConsumed} color="#ef4444"/>
              </>}
              {user.role==="internal" && (user.area==="hr"||user.area==="support") && <>
                <MetricCard label={t.teamSize} value={[...new Set(visibleProjects.flatMap(p=>p.team||[]))].length} sub={t.activeMembers}/>
                <MetricCard label={t.openTickets} value={allTickets.filter(tk=>tk.status==="open").length} sub={t.awaitingRes} color="#ef4444"/>
                <MetricCard label={t.projectsMonitored} value={visibleProjects.length} sub={t.monitored} color="#818cf8"/>
                <MetricCard label={t.resolved} value={allTickets.filter(tk=>tk.status==="resolved").length} sub={t.thisPeriod} color="#10b981"/>
              </>}
            </div>
                ...
                <div key={proj.id} style={{...S.card,cursor:"pointer"}} onClick={()=>{setActive(proj.id);setView("chat");}}>
                  ...
                  {proj.activity?.length>0 && (
                    <div style={{marginTop:10,paddingTop:8,borderTop:"1px solid #1e293b"}}>
                      <div style={{fontSize:11,color:"#475569",marginBottom:4}}>{t.recentActivity}</div>
        ...
        {view==="chat" && (
          <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
            <div style={{padding:"12px 20px",borderBottom:"1px solid #1e293b",background:"#0d1526",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <div style={{fontWeight:700,fontSize:14,color:"#f1f5f9"}}>{user.role==="internal" ? getAreaLabel(user.area) : project?.name}</div>
                <div style={{fontSize:11,color:"#64748b",marginTop:1}}>{project?.docs?.length||0} {t.docsIndexed} · {messages.length} {t.turns}</div>
              </div>
        ...
        {view==="data" && user.role!=="client" && (
          <div style={S.panel}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:8,marginBottom:20}}>
              {[{name:"ClickUp",icon:"🎯",color:"#7c3aed",desc:"Tasks & sprints"},{name:"GitLab",icon:"🦊",color:"#fc6d26",desc:"Repos & issues"},{name:"GitHub",icon:"⚫",color:"#6e40c9",desc:"Code & PRs"},{name:"Slack",icon:"💬",color:"#4a154b",desc:"Messages"},{name:"Wiki.js",icon:"📖",color:"#1976d2",desc:"Docs"},{name:"Jira",icon:"🔵",status:"soon",color:"#0052cc",desc:"Coming soon"}].map(tool=>(
                <div key={tool.name} style={{background:"#1e293b",border:"1px solid #334155",borderRadius:10,padding:"12px",textAlign:"center"}}>
                  ...
                  {tool.status==="soon" ? <span style={{fontSize:10,color:"#475569"}}>{t.comingSoon}</span> : <button style={{...S.smBtn,fontSize:10,padding:"3px 10px",borderColor:tool.color,color:tool.color}} onClick={()=>alert(`${tool.name}: paste your workspace URL and API token to sync into Sentinel.`)}>{t.connectBtn2}</button>}
                </div>
              ))}
            </div>
        ...
        {view==="admin" && user.role==="admin" && (
          <div style={S.panel}>
            ...
            <div style={{...S.card,marginBottom:12}}>
              <div style={S.sectionTitle}>{t.createProject}</div>
              <div style={{display:"flex",gap:8}}>
                <input style={{...S.input,flex:1}} placeholder={t.projectNamePh} value={newProjName} onChange={e=>setNewProjName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&(()=>{if(!newProjName.trim())return;const id=newProjName.toLowerCase().replace(/\s+/g,"-");setProjects(p=>({...p,[id]:{id,name:newProjName,client:"New Client",color:"#6366f1",status:"on-track",health:100,budget:0,spent:0,milestones:[],team:[],docs:[],tickets:[],activity:[]}}));setActive(id);setNewProjName("");})()}/>
                <button onClick={()=>{if(!newProjName.trim())return;const id=newProjName.toLowerCase().replace(/\s+/g,"-");setProjects(p=>({...p,[id]:{id,name:newProjName,client:"New Client",color:"#6366f1",status:"on-track",health:100,budget:0,spent:0,milestones:[],team:[],docs:[],tickets:[],activity:[]}}));setActive(id);setNewProjName("");}} style={S.smBtn}><I.Plus/> {t.createBtn}</button>
              </div>
            </div>
            <div style={{...S.card,marginBottom:12}}>
